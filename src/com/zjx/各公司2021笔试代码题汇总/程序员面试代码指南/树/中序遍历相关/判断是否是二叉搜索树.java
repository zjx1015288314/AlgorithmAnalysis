...
public class 判断是否是二叉搜索树 {
    ...
    /**
     * 树形DP，很多场景比如验证是否是平衡二叉树等都会用到   自定义一个返回类ReturnRes
     * @ref{是否是平衡二叉树}
     */
    public boolean isValidBST6(TreeNode root) {
        ReturnRes res = isValidBSTHelper(root);
        return res.isBST;
    }

    public ReturnRes isValidBSTHelper(TreeNode root) {
        if(root == null) {
            return new ReturnRes(Long.MIN_VALUE, Long.MAX_VALUE, true);
        }

        ReturnRes leftRes = isValidBSTHelper(root.left);
        ReturnRes rightRes = isValidBSTHelper(root.right);

        boolean flag = root.val > leftRes.maxNum && root.val < rightRes.minNum;

        boolean isBST = leftRes.isBST && rightRes.isBST && flag;
        long maxNumOfBST = Math.max(leftRes.maxNum, Math.max(rightRes.maxNum, root.val));
        long minNumOfBST = Math.min(leftRes.minNum, Math.min(rightRes.minNum, root.val));
        return new ReturnRes(maxNumOfBST, minNumOfBST, isBST);
    }

    class ReturnRes {
        long maxNum;  //防止节点的val值是上下限时无法判定,改用long
        long minNum;
        boolean isBST;
        public ReturnRes(long maxNum, long minNum, boolean isBST){
            this.maxNum = maxNum;
            this.minNum = minNum;
            this.isBST = isBST;
        }
